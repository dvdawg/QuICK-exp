"""Power Rabi: sweep qubit pulse gain at fixed pulse duration."""
...
Z_LENGTH_US = 0.2
Z_SETTLE_US = 5.0
USE_ACCEPTED_RESONATOR_FLUX_FIT = True # Used only when the accepted-fit option above is False.
READOUT_FREQUENCY_MHZ = 6883.11
Q_FREQUENCY_MHZ = 5603.91
Q_GAIN = np.linspace(0.0, 0.8, 161)
Q_LENGTH_US = 0.115
EXTRA_PI_PULSES = 0
R_POWER_DB = -30.0
R_LENGTH_US = 2.0
...
def main():
    ...
    return run_experiment(
        PROJECT_ROOT,
        experiment="rabi",
        preset="rabi_amplitude",
        title=f"{z_tag(Z_GAIN)}_power_rabi_q{number_tag(Q_FREQUENCY_MHZ)}",
